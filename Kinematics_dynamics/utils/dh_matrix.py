...
def transformation_matrix(dh_array):
    ...
    Args:
        DH parameters : List of all dh params
    Returns:
...
def CalculateJacobians(Z_j, PO_j,O_j):
    """
    Computes the Jacobian matrix  of a manipulator given the joint angles using DH parameters.
    
    Args:
        DH parameters : List of all rotation/linear displacement axis, postion of link frame, position of COM 
    Returns:
    """ 
    num_joints = len(PO_j)
